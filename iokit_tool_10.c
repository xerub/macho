...
static void *
flatten_kext(const unsigned char *p, size_t size, size_t *outsz)
{
    unsigned i, j;
    const struct mach_header *hdr = (struct mach_header *)p;
    const uint8_t *q;
    int is64 = 0;
    uint8_t *out;
    size_t total, trailing;
    addr_t min = -1ULL, max = 0, fmx = 0;
    addr_t delta = 0;
    ...
    for (i = 0; i < hdr->ncmds; i++) {
        ...
        if (cmd->cmd == LC_SEGMENT) {
            ...
            if (fmx < seg->fileoff + seg->filesize) {
                fmx = seg->fileoff + seg->filesize;
            }
        }
        if (cmd->cmd == LC_SEGMENT_64) {
            const struct segment_command_64 *seg = (struct segment_command_64 *)q;
    ...
    memcpy(out + max - min, p + size - trailing, trailing);
    ...
    for (i = 0; i < hdr->ncmds; i++) {
        ...
        if (cmd->cmd == LC_SEGMENT) {
            const struct segment_command *seg = (struct segment_command *)q;
            memcpy(out + seg->vmaddr - min, p + seg->fileoff, seg->filesize);
    ...
    q = out + sizeof(struct mach_header) + is64;
    for (i = 0; i < hdr->ncmds; i++) {
        const struct load_command *cmd = (struct load_command *)q;
        if (cmd->cmd == LC_SEGMENT) {
            ...
            for (j = 0; j < seg->nsects; j++) {
                sec[j].offset = sec[j].addr - min;
            }
            seg->fileoff = seg->vmaddr - min;
        }
        if (cmd->cmd == LC_SEGMENT_64) {
        ...
        if (cmd->cmd == LC_DYSYMTAB) {
            struct dysymtab_command *dys = (struct dysymtab_command *)q;
            if (dys->tocoff) dys->tocoff += delta;
            if (dys->modtaboff) dys->modtaboff += delta;
            if (dys->extrefsymoff) dys->extrefsymoff += delta;
            if (dys->indirectsymoff) dys->indirectsymoff += delta;
            if (dys->extreloff) dys->extreloff += delta;
            if (dys->locreloff) dys->locreloff += delta;
        }
        q = q + cmd->cmdsize;
    }
...
static void
parse_one_ex(size_t offset, addr_t base, sqlite3 *db)
{
    ...
    if (!addr) {
        ...
        if (!addr) {
            addr = get_sect_data(kernel + offset, kernel_size - offset, "__DATA_CONST", "__got", &size);
        }
    }
    if (addr && size) {
        addr += offset;
    ...
    if (__ZN11OSMetaClassC2EPKcPKS_j) {
        ...
        if (!addr) {
            ...
            if (!addr) {
                addr = get_sect_data(kernel + offset, kernel_size - offset, "__TEXT_EXEC", "__stubs", &size);
            }
        }
        if (addr && size) {
            addr += offset;
    ...
    if (stub__ZN11OSMetaClassC2EPKcPKS_j) {
        ...
        if (!addr) {
            ...
            if (!addr) {
                addr = get_sect_data(kernel + offset, kernel_size - offset, "__DATA_CONST", "__mod_init_func", &size);
            }
        }
        if (addr && size) {
            addr += offset;
...
parse_one(size_t offset, addr_t base, sqlite3 *db)
{
    ...
    kernel = flatten_kext(kernel + offset, kernel_size - offset, &kernel_size);
    assert(kernel);
    parse_one_ex(0, base, db);
    kernel = k;
    kernel_size = ks;
}
